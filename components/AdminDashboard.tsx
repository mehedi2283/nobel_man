...
const StatCard = ({ title, value, icon }: { title: string, value: number, icon: React.ReactNode }) => (
   <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 flex items-center justify-between transition-transform hover:scale-[1.02]">
       <div>
...
const Input = ({ label, name, value, onChange, placeholder, type = "text" }: any) => (
    <div className="space-y-2">
        ...
        <input 
            ...
            value={value}
            onChange={onChange}
            className="w-full px-4 py-3 bg-white text-gray-900 border border-gray-300 rounded-lg focus:outline-none focus:border-black transition-all font-medium text-sm"
    ...
    <div className="space-y-2">
        ...
        <div className="flex gap-3">
            <input 
                ...
                value={value}
                onChange={onChange}
                className="flex-1 px-4 py-3 bg-white text-gray-900 border border-gray-300 rounded-lg focus:outline-none focus:border-black transition-all font-medium text-sm"
                placeholder={placeholder || "https://..."}
            />
            {value && (
                <div className="w-12 h-12 rounded-lg border border-gray-200 bg-gray-50 overflow-hidden flex-shrink-0 relative group">
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ projects, onSaveProject, onDeleteProject, onReorderProjects, onProjectUpdate, onProfileUpdate, onLogout, onRefreshRequests, homeLogo }) => {
  ...
  useEffect(() => {
    const pollInterval = setInterval(async () => {
        try {
            ...
            // Check for profile updates (including Treat toggle from other tabs/users)
            const pData = await projectService.getProfile();
            if (pData) {
                setProfileData(prev => ({...prev, showTreatModal: pData.showTreatModal}));
            }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const finalProject: Project = {
        ...
        // Preserve existing fields if editing, or set defaults
      ...
      <div className="container mx-auto px-6 py-8 flex-1 pb-24 md:pb-8">
        <div className="flex flex-col md:flex-row gap-8 items-start">
            <div className="hidden md:flex w-64 flex-col gap-2 shrink-0 sticky top-24">
                <button onClick={() => { setActiveTab('overview'); resetForm(); }} className={getTabClass('overview')}><LayoutDashboard size={20} />Overview</button>
                <button onClick={() => { setActiveTab('inbox'); resetForm(); }} className={getTabClass('inbox')}><Mail size={20} />Inbox{totalUnreadInbox > 0 && (<span className="ml-auto bg-red-500 text-white text-xs px-2 py-0.5 rounded-full font-bold">{totalUnreadInbox}</span>)}</button>
                <button onClick={() => { setActiveTab('list'); resetForm(); }} className={getTabClass('list')}><LayoutGrid size={20} />All Projects<span className="ml-auto bg-gray-100 text-gray-900 text-xs px-2 py-0.5 rounded-full font-bold">{projects.length}</span></button>
                <button onClick={() => { setActiveTab('form'); resetForm(); }} className={getTabClass('form')}><Plus size={20} />Add New Project</button>
                <button onClick={() => { setActiveTab('logos'); resetForm(); }} className={getTabClass('logos')}><Briefcase size={20} />Client Logos</button>
                <button onClick={() => { setActiveTab('profile'); resetForm(); }} className={getTabClass('profile')}><User size={20} />Edit Profile</button>
            </div>
                    ...
                    {activeTab === 'overview' && (
                        <motion.div key="overview" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="space-y-8">
                           <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                               ...
                               <div className={`p-6 rounded-2xl shadow-sm border transition-all duration-300 flex items-center justify-between cursor-pointer ${profileData.showTreatModal ? 'bg-orange-50 border-orange-200' : 'bg-white border-gray-200'}`} onClick={handleTreatToggle}>
                                   <div>
                                       <p className={`text-sm font-medium mb-1 ${profileData.showTreatModal ? 'text-orange-600' : 'text-gray-500'}`}>Treat Mode</p>
                                       <h3 className={`text-lg font-bold flex items-center gap-2 ${profileData.showTreatModal ? 'text-orange-900' : 'text-gray-900'}`}>
                                           {profileData.showTreatModal ? 'Active' : 'Inactive'}
                                           {isTreatToggling && <Loader2 size={16} className="animate-spin text-gray-400" />}
                                       </h3>
                                   </div>
                                   <div className={`w-12 h-12 rounded-full flex items-center justify-center transition-colors ${profileData.showTreatModal ? 'bg-orange-100 text-orange-600' : 'bg-gray-50 text-gray-400'}`}>
                                       <Utensils size={20} />
                                   </div>
                               </div>
                           </div>
                                ...
                                <motion.div layout className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                                    ...
                                    <motion.div layout className="relative flex items-center gap-2">
                                        <AnimatePresence mode="popLayout">
                                            {totalUnreadInbox > 0 && (
                                                <motion.button layout initial={{ opacity: 0, scale: 0.9, width: 0 }} animate={{ opacity: 1, scale: 1, width: "auto" }} exit={{ opacity: 0, scale: 0.9, width: 0 }} transition={{ type: "spring", stiffness: 300, damping: 25 }} onClick={handleMarkAllRead} className="px-4 py-2 bg-white border border-gray-200 text-gray-600 rounded-2xl hover:bg-gray-50 hover:text-black hover:border-gray-300 transition-colors text-sm font-medium flex items-center gap-2 whitespace-nowrap shadow-sm h-[40px] overflow-hidden"><CheckSquare size={16} className="shrink-0" /><span className="hidden sm:inline whitespace-nowrap">Mark All Read</span><span className="sm:hidden whitespace-nowrap">Read All</span></motion.button>
                                            )}
                                        </AnimatePresence>
                                        <motion.button layout className={`relative cursor-pointer border rounded-2xl px-5 py-2 flex items-center gap-3 transition-all h-[40px] ${dateFilter ? 'bg-black text-white border-black' : 'bg-white border-gray-200 hover:border-gray-400 text-gray-600'}`} onClick={() => setShowDatePicker(!showDatePicker)}><Filter size={14} className={dateFilter ? "text-white" : "text-gray-400"} /><span className="text-sm font-medium whitespace-nowrap">{dateFilter ? new Date(dateFilter).toLocaleDateString() : 'Filter by Date'}</span>{dateFilter && (<span onClick={(e) => { e.stopPropagation(); setDateFilter(''); }} className="ml-2 w-5 h-5 bg-white text-black rounded-full flex items-center justify-center hover:bg-gray-200 transition-colors"><X size={12} /></span>)}</motion.button>
